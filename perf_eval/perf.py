...
def script_server(opti=0):
    ...
    if opti == 0:
        process = subprocess.Popen(cmd + ['./server', '-j', THREAD, '-s', FSIZE, '-p', SERVER_PORT], stdout=subprocess.PIPE, cwd=PARENT_PATH)
    elif opti == 1:
        process = subprocess.Popen(cmd + ['./server-inline', '-j', THREAD, '-s', FSIZE, '-p', SERVER_PORT], stdout=subprocess.PIPE, cwd=PARENT_PATH)
    elif opti == 2:
        process = subprocess.Popen(cmd + ['./server-unroll', '-j', THREAD, '-s', FSIZE, '-p', SERVER_PORT], stdout=subprocess.PIPE, cwd=PARENT_PATH)
    else:
        process = subprocess.Popen(cmd + ['./server-optim', '-j', THREAD, '-s', FSIZE, '-p', SERVER_PORT], stdout=subprocess.PIPE, cwd=PARENT_PATH)
    return process
...
if __name__ == "__main__":
    ...
    for ksize in KSIZES:
        ...
        for opti in OPTIS:
            ...
            init_file(filename_rtime, "fsize,ksize,request_rate,thread,opti,rtim,iteration\n")
            for it in range(NB_ITERATIONS):
                ...
                response_times = (float(rtime) for rtime in re.findall("response_time=(\d+.?\d*)", client_output).group(1))

                # TODO: parse outputs
                measurements = {} # {perf_arg: value}
                for perf_arg in PERF_ARGS:
                    measurements[perf_arg] = float(re.search("(\d+(?:.?\d)*)\s+"+perf_arg, server_output).group(1))
